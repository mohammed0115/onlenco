...
def _call_ai(text: str, context: dict | None) -> dict | None:
    ...
    try:
        # Centralised ai_usage wrapper (Prompt 12A.1): lesson-assistant
        # error analysis. Function-calling passed via extra_payload.
        from ai_usage import constants as AC
        from ai_usage.services import ai_client

        data = ai_client.chat(
            payload["messages"], feature=AC.FEATURE_LESSON_ASSISTANT,
            model=settings.AI_MODEL,
