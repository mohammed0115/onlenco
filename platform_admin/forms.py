...
class PlacementQuestionForm(forms.ModelForm):
    ...
    `code` is auto-generated by `PlacementQuestion.save()` from the
    `(question_type, topic)` pair, so it isn't an editable field — the
    template shows it as read-only when editing an existing row.

    `options` / `scoring_rubric` are JSON fields — Django renders them
    as a textarea; admins paste valid JSON.
    """
